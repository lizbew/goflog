...
func CreatePageStoreKey(c appengine.Context, id int64) *datastore.Key {
	return datastore.NewKey(c, "Page", "", id, nil)
}
func SavePage(c appengine.Context, id int64, page *Page) int64 {
	...
	var err error
	if id > 0 {
		k = CreatePageStoreKey(c, id)

		if err := datastore.Get(c, k, &savedPage); err == nil {
...
func DeletePageByID(c appengine.Context, id int64) {
	datastore.Delete(c, CreatePageStoreKey(c, id))
}
